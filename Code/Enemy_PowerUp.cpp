...
Enemy_PowerUp::Enemy_PowerUp(int x, int y) : Enemy(x, y)
{
	...
	*/
	...
	fly_5.PushBack({ 105,315,20,26 }); // (left,up) - move
	fly_5.speed = 0.4f;

	act[LEFT_UP] = &fly_5;

	fly_6.PushBack({ 1,360,21,21 }); // 3rd turn
	fly_6.PushBack({ 24,361,25,18 });
	fly_6.PushBack({ 49,362,26,18 });
	fly_6.PushBack({ 102,362,25,18 });
